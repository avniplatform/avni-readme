...
* Update Freshdesk tickets for the released cards — from the release card list already generated in step 7.1, pick the cards that originated from support tickets (the card references the FD ticket number), reply on each ticket that the fix/feature is part of this release (include the release notes link), and inform the support team to take over verification and closure with the organisation from there
